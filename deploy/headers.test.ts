...
describe('build-time https guard', () => {
  ...
  // An unset endpoint is no longer a build-time question: the image bakes none
  // and reads them at start, where deploy/runtime-config.sh refuses to bring the
  // container up without one, and src/config/runtime.ts refuses to render an app
  // that has none. deploy/runtimeConfig.test.ts covers both.
  it('passes a build that bakes nothing, leaving the endpoints to runtime', () => {
