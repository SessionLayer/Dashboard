...
/**
 * Vite inlines these, so a value baked here is one no operator can change
 * afterwards - this is the last moment it can be rejected. Baking is optional:
 * the container image ships none and reads its endpoints at start instead, where
 * deploy/runtime-config.sh applies this same rule to the values that arrive
 * there. deploy/runtimeConfig.test.ts runs both over one table of cases.
 */
